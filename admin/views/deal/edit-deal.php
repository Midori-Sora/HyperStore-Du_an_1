...
<body>
                    ...
                    <form method="POST">
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="pro_id" name="pro_id" required
                                onchange="calculateDiscountedPrice(document.getElementById('discount').value)">
                                <option value="">Chọn sản phẩm</option>
                                <?php foreach ($products as $product): ?>
                                    <option value="<?= $product['pro_id'] ?>" data-price="<?= $product['price'] ?>"
                                        <?= ($product['pro_id'] == $deal['pro_id']) ? 'selected' : '' ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" class="form-control" id="discount" name="discount"
                                    value="<?= htmlspecialchars($deal['discount']) ?>" required min="0" max="100"
                                    onchange="calculateDiscountedPrice(this.value)">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="start_date" name="start_date"
                                value="<?= htmlspecialchars($deal['start_date']) ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="end_date" class="form-label">Ngày kết thúc</label>
                            <input type="date" class="form-control" id="end_date" name="end_date"
                                value="<?= htmlspecialchars($deal['end_date']) ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="status" class="form-label">Trạng thái</label>
                            <select class="form-select" id="status" name="status">
                                <option value="1" <?= $deal['status'] ? 'selected' : '' ?>>Hoạt động</option>
                                <option value="0" <?= !$deal['status'] ? 'selected' : '' ?>>Không hoạt động</option>
                            </select>
                        </div>
                        <div class="mt-4">
    ...
    <script>
        ...
        // Tính toán giá khi trang vừa load
        document.addEventListener('DOMContentLoaded', function() {
            calculateDiscountedPrice(<?= $deal['discount'] ?>);
        });
